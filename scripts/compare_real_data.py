...
Also reports GRIN's envelope/reconstruction deviance for each real observer: a high value
means the network's own fitted parameters don't reproduce this matrix well, most likely
because it falls outside the region the training prior populated (extreme sensitivity,
an unusual trial-count regime, etc.) rather than because no Gaussian-GRT parameters could
fit it at all -- see src/inference/ood.py's module docstring for why that distinction
matters. Treat a high value as "inspect this observer's fit before trusting it," not as a
formal test that GRT fails to describe them.
...
# PROVISIONAL, uncalibrated: picked by eye on dev-scale data, not validated against
# operating characteristics (false-warning/detection rate across trial-count regime,
# row imbalance, distance from z_max/r_max, permutation type) on the production model.
# Treat the flag below as "worth a second look," not a pass/fail verdict.
ENVELOPE_THRESHOLD = 40.0
...
def main():
    ...
    for i, name in enumerate(names):
        ...
        flag = "ok" if dev[i] < ENVELOPE_THRESHOLD else "CHECK — outside the trained envelope (provisional threshold)"
        print(f"    envelope check (deviance) {dev[i]:.1f}  [{flag}]")
    ...
    if (dev >= ENVELOPE_THRESHOLD).any():
        bad = [names[i] for i in np.where(dev >= ENVELOPE_THRESHOLD)[0]]
        print(f"=== {len(bad)} observer(s) flagged as outside the trained envelope: {', '.join(bad)} ===")
    ...
    ax[1].barh(y, dev, color=[BLUE if d < ENVELOPE_THRESHOLD else RED_DEEP for d in dev])
    ax[1].axvline(ENVELOPE_THRESHOLD, color=INK, lw=1, ls=(0, (3, 3)))
    ax[1].set_yticks(y); ax[1].set_yticklabels([]); ax[1].invert_yaxis()
    ax[1].set_xlabel("envelope deviance")
    ax[1].set_title(f"Envelope check (rose = above {ENVELOPE_THRESHOLD:.0f}, treat with caution)",
                    fontsize=10)
