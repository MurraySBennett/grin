...
if __name__ == '__main__':
    ...
    y_means_train, y_means_val, y_means_test = (
        y_means_train_raw, #(y_means_train_raw - means_mean) / means_std,
        y_means_val_raw, #(y_means_val_raw - means_mean) / means_std,
        y_means_test_raw, #(y_means_test_raw - means_mean) / means_std
    )
    y_covs_train, y_covs_val, y_covs_test = (
        y_covs_train_raw, #(y_covs_train_raw - covs_mean) / covs_std,
        y_covs_val_raw, #(y_covs_val_raw - covs_mean) / covs_std,
        y_covs_test_raw #(y_covs_test_raw - covs_mean) / covs_std
    )
    y_crit_train, y_crit_val, y_crit_test = (
        y_crit_train_raw, #(y_crit_train_raw - crit_mean) / crit_std,
        y_crit_val_raw, #(y_crit_val_raw - crit_mean) / crit_std,
        y_crit_test_raw #(y_crit_test_raw - crit_mean) / crit_std
    )
    ...
    X_train = X_train_raw #(X_train_raw - X_mean) / X_std
    X_val = X_val_raw #(X_val_raw - X_mean) / X_std
    X_test = X_test_raw #(X_test_raw - X_mean) / X_std
    ...
    for MODEL_FILE in MODEL_FILES:
        ...
        for stage_idx, (accuracy_range, stage_models) in enumerate(STAGED_CURRICULUM):
            ...
            if is_multi_task:
                if stage_idx > 0:
                    previous_model_path = os.path.join(MODEL_RESULTS_DIR, f"{config['model_name']}_{stage_idx}.h5")
                    model = tf.keras.models.load_model(
                        previous_model_path,
                        custom_objects=config['losses']
                    )

                train_targets = {'classification_output': y_cls_train_stage,
                                 'means_output': y_mean_train_stage,
                                 'cov_output': y_cov_train_stage,
                                 'crit_output': y_crit_train_stage}
                val_targets = {'classification_output': y_cls_val_stage,
                               'means_output': y_mean_val_stage,
                               'cov_output': y_cov_val_stage,
                               'crit_output': y_crit_val_stage}
                history = model.fit(
                ...
                # Classification
                if stage_idx > 0:
                    previous_model_path = os.path.join(MODEL_RESULTS_DIR, f"{config['model_name']}_cls_{stage_idx}.h5")
                    cls_model = tf.keras.models.load_model(
                        previous_model_path#,
                        # custom_objects={'classification_loss': classification_loss}
                    )
                    previous_model_path = os.path.join(MODEL_RESULTS_DIR, f"{config['model_name']}_reg_{stage_idx}.h5")
                    reg_model = tf.keras.models.load_model(
                        previous_model_path,
                        custom_objects=config['reg_losses']
                    )
